...
describe("Logger Middleware", () => {
  ...
  it("should log outgoing response with status and duration", async () => {
    ...
    const responseLog = allCalls.find(
      (log: unknown) =>
        typeof log === "string" && log.includes("<-- GET /test"),
    );
  ...
  it("does not log exact /health requests", async () => {
  ...
  it("does not log exact /ready requests", async () => {
  ...
  it("still logs paths that merely begin with /health", async () => {
    app.get("/healthcheck-imposter", (c) => c.json({ ok: true }));

    await app.request("/healthcheck-imposter");

    expect(consoleSpy).toHaveBeenCalledWith(
      expect.stringContaining("--> GET /healthcheck-imposter"),
    );
  });
});
